...
export async function cartCreate(variantGid, quantity, locale = 'en') {
  const query = `
  ...
  try {
    const data = await shopifyData(query, {
      input: {
        ...
        attributes: [{key: 'locale', value: locale}],
      },
    })
...
// Sincroniza el atributo `locale` del carrito (la Function B2B lo lee para emitir
// el mensaje del descuento en el idioma del comprador en el checkout).
export async function cartLocaleUpdate(cartId, locale) {
  const query = `
  ...
  try {
    ...
    return data.cartAttributesUpdate.cart ?? null
  } catch (err) {
    return {error: err}
  }
}
...
export async function cartCreateMultiple(lines, locale = 'en') {
  const query = `
    mutation cartCreate($input: CartInput!) {
      cartCreate(input: $input) {
        cart { ${CART_LINES_FRAGMENT} }
...
export async function getCart(cartId) {
  const query = `
    query getCart($cartId: ID!) {
      cart(id: $cartId) { ${CART_LINES_FRAGMENT} }
    }
  `
  try {
    const data = await shopifyData(query, {cartId})
...
const COLLECTION_META_QUERY = `
  query CollectionMeta($handle: String!, $language: LanguageCode!, $country: CountryCode!)
...
export async function getCollectionMeta(handle, {language = 'EN', country = 'ES'} = {}) {
  const data = await shopifyData(COLLECTION_META_QUERY, {handle, language, country})
...
export async function getAllProductsForFilters(
  handle,
  filters = [],
  {sortKey = 'COLLECTION_DEFAULT', reverse = false, language = 'EN', country = 'ES'} = {},
) {
  const all = []
  let after = null
  let hasNext = true
...
export async function getAllShopProducts(
  filters = [],
  {sortKey = 'RELEVANCE', reverse = false, language = 'EN', country = 'ES'} = {},
  query = '',
) {
  const all = []
  let after = null
...
const PRODUCT_DETAIL_QUERY = `
  ...
  query ProductDetail($handle: String!, $language: LanguageCode!, $country: CountryCode!)
  @inContext(language: $language, country: $country) {
    product(handle: $handle) { ...ProductDetail }
  }
`

export async function getProductDetail(handle, {language = 'EN', country = 'ES'} = {}) {
  const data = await shopifyData(PRODUCT_DETAIL_QUERY, {handle, language, country})
  return data?.product ?? null
}

const PRODUCT_CARD_BY_HANDLE_QUERY = `
  query ProductCardByHandle($handle: String!, $language: LanguageCode!, $country: CountryCode!)
  @inContext(language: $language, country: $country) {
...
export async function getProductCards(handles, {language = 'EN', country = 'ES'} = {}) {
  if (!Array.isArray(handles) || handles.length === 0) return []
  const results = await Promise.all(
    handles.map(async (handle) => {
      try {
        const data = await shopifyData(PRODUCT_CARD_BY_HANDLE_QUERY, {handle, language, country})
        return data?.product ?? null
      } catch {
        return null
      }
...
const PRODUCT_CARD_FULL_BY_HANDLE_QUERY = `
  ${PRODUCT_CARD_FRAGMENT}
  query ProductCardFullByHandle($handle: String!, $language: LanguageCode!, $country: CountryCode!)
  @inContext(language: $language, country: $country) {
    product(handle: $handle) { ...ProductCard }
  }
`

export async function getProductCardsByHandles(handles, {language = 'EN', country = 'ES'} = {}) {
  if (!Array.isArray(handles) || handles.length === 0) return []
  const results = await Promise.all(
    handles.map(async (handle) => {
      const data = await shopifyData(PRODUCT_CARD_FULL_BY_HANDLE_QUERY, {handle, language, country})
      return data?.product ?? null
    }),
  )
